...
contract_address = os.getenv('CONTRACT_ADDRESS', '0x11564897bdAf32Fa8997167A6e4A1832345Fe2e1')
...
class ElectionRoundAdmin(admin.ModelAdmin):
    ...
    def save_model(self, request, obj, form, change):
        ...
        if not change:
            # Add election round to blockchain
            tx = contract.functions.addElectionRound(
                obj.name,
                int(obj.start_date.timestamp()),
                int(obj.end_date.timestamp())
            ).build_transaction({
    ...
    def save_model(self, request, obj, form, change):
        ...
        if not change:
            try:
                ...
                # Optional: Check logs if your contract emits events
                if tx_receipt.logs:
                    for log in tx_receipt.logs:
                        print(f"Log: {log}")

                # Verify if the candidate was actually added
                candidate_id = 1  # Replace with actual candidate ID if needed
                candidate_info = contract.functions.getCandidate(
                    int(obj.election_round.id),  # Ensure the ID is an integer
                    candidate_id  # Provide the candidate ID
                ).call()
                ...
                if candidate_info:
                    print(f"Candidate Details: ID: {candidate_info[0]}, Name: {candidate_info[1]}, Count: {candidate_info[2]}")
...
# Ensure Candidate model is registered only once
try:
    admin.site.register(Candidate, CandidateAdmin)
except admin.sites.AlreadyRegistered:
    pass

# Register Vote model if needed
